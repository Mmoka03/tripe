...
/**
 * 실제 스프링 백엔드에서 도시 분석 정보를 조회합니다.
 * GET /api/destinations/{cityId}/analysis
 */
export async function fetchDestinationAnalysis(cityId: string): Promise<DestinationAnalysis> {
    try {
        // 1. 실제 API 호출
        const response = await api.get<ApiResponse<DestinationAnalysis>>(
            `/api/destinations/${cityId}/analysis`
        );
        ...
        if (response.data.success) {
            ...
            throw new Error('서버 응답은 성공했으나 데이터 로드에 실패했습니다.');
        }
    } catch (error: any) {
        console.error('API 호출 중 오류 발생:', error);

        // 에러 발생 시 사용자에게 보여줄 에러 메시지를 던지거나
        // 필요하다면 위에서 정의한 defaultData를 return 하도록 설계할 수 있습니다.
        throw new Error(error.response?.data?.message || '네트워크 오류가 발생했습니다.');
    }
}
...
// /**
//  * 여행 API 모의 함수
//  * 실제 환경에서는 Spring 백엔드의 REST API를 호출한다.
//  *
//  * API 엔드포인트 가정:
